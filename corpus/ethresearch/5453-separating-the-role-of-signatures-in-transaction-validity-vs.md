...
source: ethresearch
topic_id: 5453
title: Separating the role of signatures in transaction validity vs slashing
author: ldct
date: "2019-05-14"
category: Layer 2 > Plasma
tags: []
url: https://ethresear.ch/t/separating-the-role-of-signatures-in-transaction-validity-vs-slashing/5453
views: 1961
likes: 1
posts_count: 1
---
...
thanks to [@gakonst](/u/gakonst) for review
...
Let P = \{\mathrm{Alice}, \mathrm{Bob}, \mathrm{Carol}, \mathrm{Dave}\}. Suppose that the in-flight transaction T consumes UTXOs owned by Alice and Bob [1] and produces UTXOs owned by Carol and Dave, and T is withheld.  We do not assume that any member of P can cooperate with any other. It follows that an exit must be able to be initiated by some single member of $P4. There are two cases: we can disallow Alice and Bob from being beneficiaries by allowing Carol or Dave to exit as if T were always in the chain (“forced include exits”), or we can allow any of them to be beneficiaries by allowing exitors to make a claim that (i.e., an exit that succeeds when) the chain either contains T or not (“limbo exit”). In either case, an individual exitor must make a claim that can could be false by no fault of their own.
...
3. Coin history consists of both type V and type S signatures, if they exist
...
[2] [Plasma Cash Minimal Atomic Swap](https://ethresear.ch/t/plasma-cash-minimal-atomic-swap/3409/4)
