...
source: ethresearch
topic_id: 20467
title: Using FRI for DA with Optimistic Correctable Commitments in Rollups
author: snjax
date: "2024-09-21"
category: Layer 2 > ZK Rollup
tags: [zk-roll-up, data-availability, fraud-proofs]
url: https://ethresear.ch/t/using-fri-for-da-with-optimistic-correctable-commitments-in-rollups/20467
views: 387
likes: 4
posts_count: 3
---
...
![fig0](https://ethresear.ch/uploads/default/original/3X/6/9/69b5279febeda88a190490a8338a91e9431cf046.svg)
...
![fig1](https://ethresear.ch/uploads/default/original/3X/a/b/aba4dab24d942f940b86fe1914cac86be03f1d1b.svg)
...
![fig2](https://ethresear.ch/uploads/default/original/3X/0/9/09b2e020e39c1b237b66c4e6c51d7b41153b6825.svg)
...
Commitment C.
2. Shard hashes \{H_j\}.
3. Opening \pi = \{a_i\} at point \chi.
4. Construction \mathcal{C} = (C, \chi, \{a_i\}).
5. Transaction Validation:

Validators check the client’s signature.
6. Verify the correctness of metadata and opening \pi.
7. Verify the correctness of \mathcal{C}.
...
11. Correctness of opening \pi and construction \mathcal{C}.
12. If all checks are successful, nodes sign the transaction, after which the transaction can be included in a block.
13. Metadata Storage:
...
**defitricks** (2024-10-19):

How does the introduction of optimistic correctable commitments enhance error correction mechanisms in data shards compared to standard commitments in rollups? ![:thinking:](https://ethresear.ch/images/emoji/facebook_messenger/thinking.png?v=12)

---

**snjax** (2024-10-21):

Current rollups use DAS, based on 2d RS commitments. This approach requires commitments with size O(\sqrt{|D|}). This 2d structure differs from native rollup commitments, so we need some additional zk interlayer.

DAS allows to prove that the network stores the whole data (this is described in “Fraud and Data Availability Proofs” article). But the issue with this approach is that we still need honest minority assumptions, otherwise, the network will store the data but make a DoS attack.

Here we use honest minority assumption only (with SPoRA to incentivize storing the data). It allows us to reduce the commitment to small fixed-sized construction stored on the consensus level (and N additional hashes, stored in decentralized DB over the nodes).

FRI commitment inside this construction is general-purpose FRI, it could be used in sum-IOP and poly-IOP in rollups with no interlayers.

So, this rollup can steamless store it’s blocks inside itself, which reaches infinite scalability possibilities.

The cost of on-chain maintenance of this construction is very cheap, so it could be used not just for DA, but also as storage for data-rich dApps, like Farcaster, and decentralized AI.
