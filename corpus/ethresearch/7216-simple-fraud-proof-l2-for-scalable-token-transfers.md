...
source: ethresearch
topic_id: 7216
title: Simple Fraud Proof L2 for Scalable Token Transfers
author: 0age
date: "2020-03-30"
category: Layer 2
tags: [layer-2, fraud-proofs]
url: https://ethresear.ch/t/simple-fraud-proof-l2-for-scalable-token-transfers/7216
views: 4161
likes: 5
posts_count: 8
---
...
- blockHeader  - The block header being checked for pending status.
...
- blockHeader  - Block header to check for pending status.
- previousBlockHeader  - Block header prior to blockHeader.
...
- transactionHashes  - Array of hashes of prefixed transactions
...
Set i = 0
- Execute a for loop with condition i
...
- rootHash  - The root hash of a merkle tree.
- leafNode  - An arbitrarily encoded leaf node.
- leafIndex  - The index of the leaf in the merkle tree.
- siblings  - The neighboring nodes of the leaf going up the merkle tree.
...
- rootHash  - The root hash of a merkle tree.
- leafNode  - The leaf node to prove inclusion of.
- newLeafNode  - The leaf node to replace leafNode with in the tree.
- leafIndex  - The index of the leaf in the merkle tree.
- siblings  - The neighboring nodes of the leaf going up the merkle tree.
...
- rootHash  - The root hash of a merkle tree.
- leafNode  - The leaf node to add to the tree.
- leafIndex  - The index of the leaf in the merkle tree.
- siblings  - The neighboring nodes of the leaf going up the merkle tree.
...
- transactionsRoot  - The root hash of a transactions merkle tree.
- transaction  - An encoded transaction of any type.
- transactionIndex  - The index of the transaction in the merkle tree.
- siblings  - The neighboring nodes of the transactions going up the merkle tree.
...
- previousSource  - Data used to prove the state prior to a given transaction.
...
- Execute a while loop with condition (nextOffset
...
- previousBlockHeader  - The block header prior to the block being challenged.
- blockHeader  - The block header being challenged.
- transactions  - Transactions buffer for the block identified by header.
...
- blockHeader  - Block header being claimed as fraudulent.
- transactions  - Transactions buffer from the block.
...
- previousBlockHeader  - The block header with a number one less than header.
- blockHeader  - The block header being challenged.
- transactions  - The transactions buffer from blockHeader.
...
- blockHeader  - The block header being challenged.
- transaction  - The transaction being claimed to have a bad source.
...
- blockHeader  - Header of the block being claimed as fraudulent.
- priorStateProof  - Proof of the state prior to executing the transaction. Type union of
- transactionProof  - Merkle proof of the transaction being challenged.
...
- blockHeader  - Header representing the block with the transaction claimed to be fraudulent.
- priorStateProof  - Proof of the state prior to executing the transaction. Type union of
- transactionProof  - Merkle proof of the transaction being challenged.
...
[@liam](/u/liam) this is implemented in Dharma’s Tiramisu.



      [github.com](https://github.com/dharma-eng/Tiramisu)




  ![image](https://ethresear.ch/uploads/default/optimized/3X/2/4/24335477c979a9367578aded6aa803d1d370e488_2_690x344.png)
...
**salaxgoalie** (2020-06-11):

![](https://ethresear.ch/user_avatar/ethresear.ch/0age/48/4488_2.png) 0age:
