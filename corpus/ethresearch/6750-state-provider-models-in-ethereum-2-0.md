...
tags: []
url: https://ethresear.ch/t/state-provider-models-in-ethereum-2-0/6750
views: 6326
likes: 24
posts_count: 7
---
...
*This post was co-authored by [@samwilsn](/u/samwilsn) and [@adietrichs](/u/adietrichs), with significant input by [@villanuevawill](/u/villanuevawill) and the Quilt team.*
...
[![](https://ethresear.ch/uploads/default/original/2X/b/bd99e506c20b051cd87469c1a4a9ee1131d69017.png)521×196 7.04 KB](https://ethresear.ch/uploads/default/bd99e506c20b051cd87469c1a4a9ee1131d69017)
...
[![](https://ethresear.ch/uploads/default/original/2X/2/2f63bed8d354de14f6e9f349db717506c751b236.png)791×204 8.92 KB](https://ethresear.ch/uploads/default/2f63bed8d354de14f6e9f349db717506c751b236)
...
[![](https://ethresear.ch/uploads/default/original/2X/6/66372219de1abfba9ef57af588b3227940498a8e.png)471×186 5.1 KB](https://ethresear.ch/uploads/default/66372219de1abfba9ef57af588b3227940498a8e)
...
Do you see an issue where in the pull model that majority of the block proposers will end up becoming state provider to itself?

---

**tvanepps** (2020-01-09):

By the term “missing state” do you mean an improperly formed witness?
...
I don’t see that as a problem. In fact, it would probably be a good thing! Miners in Eth1 currently hold the entire state.

State providers are necessary in Eth2 because validators get shuffled between shards regularly, and it isn’t feasible to expect validators to hold state for every shard.
...
Not exactly; the witness would be well-formed (i.e. in the format expected by the execution environment). Missing state is where a transaction tries to access a piece of state for which no witness has been provided.

Imagine a contract in an oversimplified EE with three state locations: `A`, `B`, and `C`, with the following logic:
...
In the block producer’s pool of pending transactions, you have the following:

1. add_value(2) with witnesses proving A == 3 && B == 5
2. add_value(3) with witnesses proving A == 3 && B == 5

Individually each transaction can run, and their witnesses are sufficient to cover the state they access. When the two transactions are run sequentially, `add_value(3)` will try to read `C`, and fail because no transaction included a witness to `C`.

---

**jrhea** (2020-01-12):

Great post! In the pull model you describe, how does the Block Proposer request state from a State Provider?  Is it a direct rpc call? If so, are there privacy concerns bc it exposes that particular block proposer’s ip address to the state provider?

[![Screenshot_20200112-102825](https://ethresear.ch/uploads/default/optimized/2X/b/bf0ffd370f9ff9dbee3de9668d65dca5f0ef0e3c_2_690x323.jpeg)Screenshot_20200112-1028252048×961 152 KB](https://ethresear.ch/uploads/default/bf0ffd370f9ff9dbee3de9668d65dca5f0ef0e3c)
...
> If so, are there privacy concerns bc it exposes that particular block proposer’s ip address to the state provider?

If this is the case, there’s a chance that both the block proposer’s and state provider’s on-chain activity is linked to an IP address and subject them to possible DoS, bribery, etc.
