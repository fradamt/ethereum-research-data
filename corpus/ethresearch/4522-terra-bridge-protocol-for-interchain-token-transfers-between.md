...
source: ethresearch
topic_id: 4522
title: "Terra-Bridge: Protocol for Interchain Token Transfers between Ethereum and Bitcoin"
author: PeterH
date: "2018-12-09"
category: Economics
tags: []
url: https://ethresear.ch/t/terra-bridge-protocol-for-interchain-token-transfers-between-ethereum-and-bitcoin/4522
views: 14065
likes: 4
posts_count: 15
---
...
[![](https://ethresear.ch/uploads/default/optimized/2X/5/5f3d996970f165457b785ac9ead9b35dd4582266_2_690x358.png)1460×758 19.4 KB](https://ethresear.ch/uploads/default/5f3d996970f165457b785ac9ead9b35dd4582266)
...
1. Validators (1 to N) queries and find new incoming transactions on foreign-bridge and send message to home-bridge to relay transfer with the following parameters:

- R - recipient address on home.
- T - transfer amount
- SIG - validator signature of transfer transaction

1. The home bridge receives the incoming message, verifies validator signature and keeps track of the signatures collected from validators. When more than N/2 validator signatures are collected for a given TX then T amount BTCT (pegged version of BTC on Ethereum in the form of token) is transferred to recipient.

**Home → Foreign Transfer Flow:**

[![](https://ethresear.ch/uploads/default/optimized/2X/d/d6edee58ff064ff2fe4181de6e9fdc78b0e4420f_2_690x334.png)1464×710 16.2 KB](https://ethresear.ch/uploads/default/d6edee58ff064ff2fe4181de6e9fdc78b0e4420f)
...
1. Validators (1 to N) listens for events and find new incoming transaction on home-bridge and send message to home-bridge to collect signatures approving the transfer with the following parameters:

- R - recipient address on foreign.
- T - transfer amount
- SIG - validator’s signature (witness) that will be used to unlock funds on the foreign-bridge

1. Once enough signatures are collected (N/2 + 1) and verified, home-bridge emits event to inform validators the transfer has been fully approved. The last validator to submit the transfer transaction to foreign-bridge along with the collected validator signatures.
2. The foreign bridge receives the incoming transfer request, verifies the collected signatures and T amount BTC is transferred to recipient.
...
Other preparation that needs to be done to pass witness to `ecrecover` is splitting it to 3 parts (v,r,s) as described in the Solidity Documentation:
...
The redeem script according to BIP-16 can be constructed using the `OP_CHECKMULTISIG` opcode in of the format:

```auto
[M pubkey-1 pubkey-2 ... pubkey-N OP_CHECKMULTISIG]
```

In order to increase the upper bound of the validator set we could utilize the SegWit’s `witnessScript`, which in Bitcoin Core 0.13.1 and up allows up to 3600 bytes of script data. Following  BIP-141, we can construct a the P2SH-P2WSH (P2SH with SegWit) multisignature redeem script using the `OP_CHECKSIG` opcode in the following format:
...
Credits to ContractLand Foundation and [@KiriKiri](/u/kirikiri) for the research and development of this protocol.

## Replies

**gakonst** (2018-12-09):

The proposal for the Ethereum contract seems straightforward.

You suggest using a multisig owned by the validators on Bitcoin and use that to lock/unlock BTC?

If the validators receive a tx that should unlock a user’s funds on Bitcoin, but they do not, do you have a way to prove this censorship and slash them, so as to get cryptoeconomic security?

---

**drstone** (2018-12-09):

Is there a usual way to do this? I’ve been thinking for some time how to prove that requests have not been relayed in a somewhat similar, but traditional federated bridge setting

---

**schemar** (2018-12-10):

![](https://ethresear.ch/user_avatar/ethresear.ch/peterh/48/2801_2.png) PeterH:

> If a validator fails to perform the relay for a transfer message, he will get slashed a portion of his deposited stake.

How can you identify this? If more than 50% of signatures are proven you would slash the other validators? Is there a time-out? How can you differentiate between “all validators are offline” and “there are no transfer requests”?

![](https://ethresear.ch/user_avatar/ethresear.ch/gakonst/48/1101_2.png) gakonst:

> If the validators receive a tx that should unlock a user’s funds on Bitcoin, but they do not, do you have a way to prove this censorship and slash them, so as to get cryptoeconomic security?

I was wondering this as well. Would the users funds be locked?
...
This proposal seems similar to the work of A. Zamyatin and D. Harz at Imperial College London. They proposed a similar framework at Bitcoin Scaling 2018 - please see [video](https://www.youtube.com/watch?v=FI9cwksTrQs&feature=youtu.be&t=6410) and [paper](https://eprint.iacr.org/2018/643).

I hope the video/paper is interesting, if you haven’t seen it before.

---

**PeterH** (2018-12-11):

The scenario you’ve described is a `home-to-foreign` transfer (i.e Bitcoin side involves transfer from validator multisig to recipient address). This process includes validators submitting signatures on the home-bridge in order to arrive consensus on which bitcoin output transaction to use for the multisig transfer, as well as formulate the transfer transaction message itself. This means we will have records on the home-bridge on which validators has participated in the transfer, and ability to identify ones that have not. A simple slashing condition could be to punish validators that have failed to submit signatures for any transfers that have passed some unit time of T (could be block number, transfer number, etc).

Note that this only works if the majority of the validators are honest, which is the security assumption of the bridge system.

---

**PeterH** (2018-12-11):

A federated version of this is much easier to do. In fact, we already have a working MVP that is permission based (i.e PoA). It becomes a lot more challenging when attempting to make it PoS mainly due to the limitations of Bitcoin scripting.

---

**PeterH** (2018-12-11):

Thanks for the question Shemar. Please let me know if the reply to Gakonst answers your question ![:slight_smile:](https://ethresear.ch/images/emoji/facebook_messenger/slight_smile.png?v=9)

---

**schemar** (2018-12-11):

Thanks [@PeterH](/u/peterh), it does for `home-to-foreign`, but how do you know validators are doing their job for `foreign-to-home`? As far as I understand the validators are only staked on home. How would home know that there is a pending transaction with locked funds on the foreign chain?

---

**PeterH** (2018-12-11):

Slashing for `foreign-to-home` works in a similar fashion, as validators needs to submit their signatures in `home-bridge` in order to perform the transfer and unlock funds. We can then determine at the home side which validators have failed to perform their duty.

---

**schemar** (2018-12-11):

How do you determine at the home bridge that validators did not fulfill their duty? What if a single attacker adds a signature for a non-existing transfer?

How would the home bridge know, whether:

a. The attacker‘s signature is invalid, slash the attacker, *or*

b. All validators except one are offline, slash all except the attacker?

Maybe I am missing something? ![:thinking:](https://ethresear.ch/images/emoji/facebook_messenger/thinking.png?v=9)

---

**PeterH** (2018-12-12):

The minimal slashing condition is the same for `foreign-to-home` and `home-to-foreign` transfers. It’s based on the bridge’s cryptoeconomic security model of that as long as the majority of the validators are honest, the bridge is secure.

i.e we do not attempt to differentiate between ‘honest’ or ‘malicious’ validators, but assumes that majority of the validators are honest. When a single validator submits a signature for a transaction no other validators are relaying, that validator will be slashed.

Another possible method is to use merkle proofs for verifying if the tx being relayed are valid. However, that will be quite expensive to do given we would need to store all block headers on the `home-bridge`.

---

**KiriKiri** (2018-12-17):

Thanks for links [@maxrobot](/u/maxrobot) . We thought about using BTC-relay, but it’s too massive to support it alone. Our solution more lightweight.

---

**moderneducation** (2018-12-26):

Its a nice article thanks for posting it…so how to interface for other platforms like ardor the

most popular third type of blockchain …is the interface is secured want some more info.

thanks

venkata maddulamurty

---

**PeterH** (2018-12-29):

Hi [@moderneducation](/u/moderneducation), I’m not sure I fully understood your question, could you elaborate?
