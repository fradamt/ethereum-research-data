...
tags: [zk-id]
url: https://ethresear.ch/t/web2-nullifiers-using-voprf/21762
views: 429
likes: 10
posts_count: 4
---
...
*Huge thanks to [@levs57](/u/levs57) for helping me on this and [Privacy & Scaling Explorations](https://pse.dev/) for supporting this project*.

## Abstract

Recent development of protocols, that allow us to make Web2 data portable & verifiable such as [ZK Email](https://prove.email/) or [TLSNotary](https://tlsnotary.org/) opens new use-cases and opportunities for us. For example, we can make proof of ownership of some [x.com](http://x.com) username or email address and verify it on-chain with ZK Email. Projects like [OpenPassport](https://www.openpassport.app/), [Anon Aadhaar](https://github.com/anon-aadhaar/anon-aadhaar) (and others) are also the case.
...
For simplicity I’ll explain the case with one node OPRF first, and in OPRF-MPC section I’ll explain how we can extend it to multiple nodes.
...
\cm_2 = r * G where G = \hashc(\text{UserID}), and r is random scalar.
...
3. Private inputs: \text{UserID}, \text{salt}, r
4. \cm_1 = \hash(\text{UserID}, \text{salt})
5. G = \hashc(\text{UserID})
6. \cm_2 = r * G
7. OPRF replies with:

\text{oprf_response} = s * \cm_2
where s is a private key of OPRF node; and also replies with proof of correctness of such multiplication, which is in this case might be a Chaum-Pedersen proof of discrete log equality (check this blog post) on that.
8. User creates zk proof with the following parameters:

Public outputs: \cm_1, \text{ nullifier}
9. Private inputs: r, \text{ UserID}, \text{ salt}, \text{ chaum_pedersen_proof}, \text{ oprf_response}
10. \cm_1 = \hash(\text{UserID}, \text{ salt})
11. G \longleftarrow \hashc(\text{UserID})
12. \chaum (\text{oprf_response})
13. \text{nullifier} \longleftarrow r^{-1} * \text{ oprf_response}
...
2. 1.2. Every node does step 3 individually: we get \text{oprf_response}_i = s_i * r * G
3. 1.3. On step 4 we verify \text{chaum_pedersen_proof} for every node
4. 1.4 We calculate \text{nullifier}_i = \text{oprf_response}_i * r^{-1}
5. 1.5 We calculate \sum_{i=1}^N\text{nullifier}_i = s * G
6. M of N MPC using linear combination for Shamir Secret Sharing:

Similar to N of N MPC, but we need only M shares
7. Interactive M of N threshold MPC: similar to 2, but nodes calculate one common response and one common Chaum-Pedersen proof. With that - we won’t need to verify separate Chaum-Pedersen proofs in zk circuit, we’ll only need to verify one, though with this scheme we’ll need to care about coordination between MPC nodes.
8. Using BLS:

3.1. Calculate common public key of all OPRF nodes by summing individual public keys
9. 3.2. The same as in N of N MPC case
10. 3.3., 3.4, 3.5. - The same as in N of N MPC case, BUT we can do it outside the circuit
11. 3.6. Verify BLS pairing in the circuit
...
**OBrezhniev** (2025-02-17):

Great post!

Small improvement proposal: instead of `hash(nullifier, AppID)` you could have `hash(nullifier, AppID, AppSessionID)`.

In this way application can have different nullifiers for each let’s say voting session, preventing user tracking between separate voting sessions in the same app.

---

**curryrasul** (2025-02-18):

Thanks. I agree, that’s a good idea!

---

**Scratch-net** (2025-02-20):

Hey, nice idea with two commitments!

Reclaim also uses threshold v-OPRF to prove a “hash” of some decrypted TLS ciphertext part inside a Gnark circuit. If you’re interested the implementation is at our github: github /reclaimprotocol/zk-symmetric-crypto/tree/main/gnark/circuits/toprf
