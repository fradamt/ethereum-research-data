...
source: ethresearch
topic_id: 19379
title: Vanilla Based Sequencing
author: Perseverance
date: "2024-04-26"
category: Layer 2
tags: []
url: https://ethresear.ch/t/vanilla-based-sequencing/19379
views: 5475
likes: 16
posts_count: 4
---
...
**Authors:** [George Spasov](https://twitter.com/GSpasov) (LimeChain), [Daniel Ivanov](https://twitter.com/danielkivanov) (LimeChain)

**Thanks for early feedback:** [Justin Drake](https://twitter.com/drakefjustin) (Ethereum Foundation), [Brecht Devos](https://twitter.com/Brechtpd) (Taiko), [Péter Garamvölgyi](https://twitter.com/thegaram33) (Scroll), [Simon Brown](https://twitter.com/orbmis) (Linea), [Cooper Kunz](https://twitter.com/cooper_kunz) (Aztec), [Santiago Palladino](https://twitter.com/smpalladino) (Aztec), [Nick Dodson](https://twitter.com/IAmNickDodson) (Fuel)
...
[![Preconfirmations-Vanilla Based Sequencing.drawio](https://ethresear.ch/uploads/default/optimized/3X/6/9/69c5d356bf06fae2d6c3594caada9f40161b067a_2_590x500.png)Preconfirmations-Vanilla Based Sequencing.drawio851×720 55.5 KB](https://ethresear.ch/uploads/default/69c5d356bf06fae2d6c3594caada9f40161b067a)
...
The mechanisms of opting-in and selection verification are the subject of an [additional research document](https://github.com/LimeChain/based-preconfirmations-research/blob/732cb92474554c2529aabc61e83b8f0934ce6adf/docs/optin-mechanics.md).
...
The list building pipeline is a subject of an [additional research document](https://github.com/LimeChain/based-preconfirmations-research/blob/732cb92474554c2529aabc61e83b8f0934ce6adf/docs/pipelines.md).
...
[![Preconfirmation Flow - Simple](https://ethresear.ch/uploads/default/original/3X/e/2/e2f50f004cff46ee36b555ec23c92fe37393a9a2.png)Preconfirmation Flow - Simple581×271 18.4 KB](https://ethresear.ch/uploads/default/e2f50f004cff46ee36b555ec23c92fe37393a9a2)
...
The mechanics of preconfirmations and their pricing are the subject of a [separate research document](https://github.com/LimeChain/based-preconfirmations-research/blob/732cb92474554c2529aabc61e83b8f0934ce6adf/docs/preconfirmations-for-vanilla-based-rollups.md).
...
[![sequencing-design-space-Value Flow.drawio](https://ethresear.ch/uploads/default/optimized/3X/0/6/06e63d56eeaa90af8ccb8d83fd2d7a7e5996f606_2_690x286.png)sequencing-design-space-Value Flow.drawio941×391 36.9 KB](https://ethresear.ch/uploads/default/06e63d56eeaa90af8ccb8d83fd2d7a7e5996f606)
...
[![hackmd-github-sync-badge](https://ethresear.ch/uploads/default/original/3X/5/d/5d16b4aeb4937574bedd09ad574ceea3125f7e0b.svg)](https://hackmd.io/JeLKP_pVQMe-_J-kpb3MkQ)
...
If we assume that the L2 block time is the same as L1, then we are likely talking about very big blocks with a very high gas limit, and hence many more transactions.  Do you know if there is a precedent for such large L2 blocks and if there may be any implications for MEV capture in this scenario? The reason I ask is that I wonder if 10x bigger blocks results in 10x MEV (I doubt it, but it’s an interesting question as it would likely affect the dynamics of the block builder market).  Conversely, have you thought about the case where L2 block time is faster than L1?  for instance, I know zkSync Era has [effectively no limit](https://www.rollup.codes/zksync-era), but they also have a ~5 second block time.

---

**Perseverance** (2024-05-09):

As you stated, the zkSync Era is the only one we’ve seen with a higher block size. The design space around “block scaling” is still quite new and very few teams have explored it. The two ends of the spectrum that I can see are:

- Scaling blocks vertically - increasing the block size as you have suggested.
- Scaling blocks horizontally - having multiple regular-sized blocks in a single L1 slot with the same proposer.

Both approaches have their specific pros and cons.

Vertically scaled blocks keep the 1:1 relation between L1 slot and L2 block, thus leading to a simpler design. Bigger blocks however come at a cost - increased processing, circuit constraints, etc. To what extent these costs are applicable and acceptable by the rollups is subjective to each rollup.

Horizontally scaled blocks allow the blocks to stay within the same size as the L1 blocks, but allow for multiple L2 blocks to happen within the timeframe of a single L1 block. This leads to exact compatibility with L1. More and quicker blocks however would require more and quicker proofs. Furthermore, the L1 proposer will be the L2 sequencer of all the blocks within their L1 slot. Similarly as vertical scaling - whether these drawbacks are applicable and acceptable by each rollup is the rollup designer decision based on their specific context.

---

**The-CTra1n** (2024-06-19):

I’ve been thinking a lot about the “vanilla” element of this proposal compared to the original [based sequencing proposal](https://ethresear.ch/t/based-rollups-superpowers-from-l1-sequencing/15016) from Justin. The protocol being proposed in this article is considerably less vanilla than pure based sequencing. It is perhaps better described as a vanilla implementation of [based preconfirmations](https://ethresear.ch/t/based-preconfirmations/17353), or alternating based sequencing with preconfirmations?
