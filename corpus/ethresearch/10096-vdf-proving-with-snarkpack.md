...
tags: []
url: https://ethresear.ch/t/vdf-proving-with-snarkpack/10096
views: 4217
likes: 14
posts_count: 6
---
...
The results on the inner proof look very promising. How many elliptic curve operations per iteration are necessary inside the Groth16 proof?

Second question, for the aggregation, what are the asymptotics in both time and memory?

I’m concerned that the number of proofs aggregated will be very larger: Assuming 0.5ns per iteration for the evaluator, and a VDF target delay of 10 epochs = 3840 seconds, we need to aggregate 768 million proofs. So an n \log n factor in the aggregation could make this a lot worse, just checking this is not the case.

---

**mmaller** (2021-07-21):

So we have 3 constraints per iteration, which should translate into 4 * 3 = 12 G1 operations and 3 G2 operations.  As it is currently implemented there are actually 15 G1 operations and 3 G2 operations per iteration because I have not taken advantage of the fact that we don’t need zero-knowledge.

I’m not sure where the 768 million proofs comes from?  Are you assuming 10,000 iterations rather than (130m / (3n + 4)) iterations?

So 0.5ns is 300 * faster than our estimates which means a total circuit complexity of about 400B constraints.  If we use 130 million gates per Groth16 proof then that becomes 3076 proofs for a minutes delay.  To make that 3840 seconds we multiply by 64 to get about 200,000 proofs to aggregate.  Have I missed something important?

Asymptotically the aggregator can run in linear time O(n).  As it is currently implemented there is an n log(n) factor coming from the method we are using to compute the KZG polynomial openings ( f(X) - f(r) ) / (X - r).  I know that there are ways of computing these polys in linear time, but I don’t know if those techniques would be easy to parallelise.

---

**dankrad** (2021-07-21):

Right, that seems correct. I just assumed n=10,000 was a practical limit when reading the post. But this is not the case.

Is the trusted setup for a Groth16 of size 130 million gates doable in practice?

---

**mmaller** (2021-07-21):

I would assume yes because Filecoin managed it for their application.  It is a pretty large setup though so we would need to allow quite a bit of time per contribution.

---

**Killari** (2022-06-07):

I think it’s quite unfortunate that proving and validating time and size the result depend on the length of the time period. This make it less usable VDF scheme to be used in smart contracts and zk-circuits for other applications. Wouldn’t it be better to design a VDF scheme that would work for various applications and not just for one. We can get such scheme if were to ditch the requirement for quantum proofs I believe?
