...
tags: []
url: https://ethresear.ch/t/going-multidimensional-an-empirical-analysis-on-gas-metering-in-the-evm/22621
views: 620
likes: 7
posts_count: 6
---
...
[![3c0abdf68fa4364964ce6215fd77eb689ec5a217e9b2a2a0806125d3cb2b0db3](https://ethresear.ch/uploads/default/optimized/3X/4/c/4cbf90ecb6c27aa292f2af75bcf57493855420f8_2_517x345.jpeg)3c0abdf68fa4364964ce6215fd77eb689ec5a217e9b2a2a0806125d3cb2b0db31536×1024 152 KB](https://ethresear.ch/uploads/default/4cbf90ecb6c27aa292f2af75bcf57493855420f8)
...
[![image](https://ethresear.ch/uploads/default/optimized/3X/3/6/366de942fdc4375be39950255c17c6960ba79715_2_690x269.png)image4920×1920 428 KB](https://ethresear.ch/uploads/default/366de942fdc4375be39950255c17c6960ba79715)
...
|  |  |
| --- | --- |
| image4920×1919 384 KB | image1920×1920 156 KB |
...
[![image](https://ethresear.ch/uploads/default/optimized/3X/6/a/6a59822948d2357fee5d33edda5fed6477dbcc0e_2_690x269.png)image4920×1920 301 KB](https://ethresear.ch/uploads/default/6a59822948d2357fee5d33edda5fed6477dbcc0e)
...
[![image](https://ethresear.ch/uploads/default/optimized/3X/6/4/64013190d4aeb8506b4b5037753d2f9d8fd1bbae_2_690x268.png)image4928×1920 431 KB](https://ethresear.ch/uploads/default/64013190d4aeb8506b4b5037753d2f9d8fd1bbae)
...
Indeed this is exactly the strategy I’ve been considering for the new L1 project I’m working on: four separate gas prices for CPU, storage, I/O, and bandwidth. However I’m not sure I’ll do this, I think I’ll stick to a single gas dimension due to the problem I’m discussing below.

> Even though the experience for users remains the same - transactions cost the same, and users continue to submit a single gas limit and maximum fee - block builders now have a more complicated optimization problem. Not only do they need to consider the MEV and transaction fees, but also how to efficiently pack blocks into the various resources.

It doesn’t look like a big problem to me, just keep piling up transactions until they exceed one of the gas dimensions.

The real problem is that the nodes will collect more fees if the block has “filled up” *all* dimensions as much as possible, and that means the network will prefer transactions whose gas cost profile resembles the proportion of the gas limits. Wildly imbalanced transactions may be discarded. How do we fix that? One way is to allow the user to propose a higher gas fee for certain dimensions, but that makes the UX much more complicated and weirder. If e.g. a CPU-heavy transaction uses 1 part of storage and 1000 parts CPU it would be strange to ask the user to pay 1000 times the market price for storage gas just to make sure the transaction is included (I’m assuming the block gas limits for CPU and storage are equal for simplicity, the example can be generalized to any proportions).

---

**jonhubby** (2025-07-15):

Good points, 71104. I’m wondering the same about skewed transactions getting priced out. Maybe a partial solution could be dynamic per-dimension base fees that adjust depending on utilization, like how EIP-1559 adjusts the base fee overall. Still complex, though. Curious to hear if anyone’s experimented with that yet.

---

**71104** (2025-07-15):

Note that the conversation about this proposal continued here: [A practical proposal for Multidimensional Gas Metering](https://ethresear.ch/t/a-practical-proposal-for-multidimensional-gas-metering/22668)
...
1. A practical proposal for Multidimensional Gas Metering - #8 by dcrapis ↩︎

---

**misilva73** (2025-07-16):

I replied to point #1 [here](https://ethresear.ch/t/a-practical-proposal-for-multidimensional-gas-metering/22668/11).
...
I mean, you’re free to do any kind of analysis you’d like of course, but every profit is a profit and if a block builder can make even just 2 more cents by making a choice rather than another what’s stopping it?

Maybe a better approach is to identify any kind of costs that could offset those profits and discourage this new MEV criterion.
