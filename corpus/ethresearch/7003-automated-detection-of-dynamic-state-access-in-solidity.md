...
tags: [stateless]
url: https://ethresear.ch/t/automated-detection-of-dynamic-state-access-in-solidity/7003
views: 5775
likes: 10
posts_count: 9
---
...
Continuing the exploration of state provider models described [earlier](https://ethresear.ch/t/state-provider-models-in-ethereum-2-0/6750), we have roughly prototyped a modification to the solidity compiler that can detect instances of DSA in smart contracts using taint analysis.
...
A short classification of some types of DSA, with solidity examples, can be found [here](https://gist.github.com/SamWilsn/369de587ac7373c8f77ed26079531671).

### Yul Language

Example code in this article is written in pseudocode loosely resembling EVM-flavored [Yul](https://solidity.readthedocs.io/en/v0.6.3/yul.html), an internal representation language used in the solidity compiler. The prototype is written as an optimization pass that operates on Yul.

### Taint Analysis

Taint analysis (or [taint checking](https://en.wikipedia.org/wiki/Taint_checking)) is a type of data flow analysis where “taint” flows from an upstream source into all downstream variables. It is commonly used in web applications, where user supplied values (like headers) are the source of taint, and variables used in database queries are verified to be taint-free.
...
[![00-tweedle](https://ethresear.ch/uploads/default/original/2X/c/cfb1612e02edf795b24d5d0942ec281670be2e12.png)00-tweedle265×443 17.9 KB](https://ethresear.ch/uploads/default/cfb1612e02edf795b24d5d0942ec281670be2e12)

## Prototype

### Implementation

Implementation source code is available for [solidity 0.5](https://github.com/SamWilsn/solidity/blob/state-taint-0.5.16/libyul/optimiser/OrderDependentStateDestroyer.cpp) and for [solidity 0.6](https://github.com/SamWilsn/solidity/blob/e00faa7d25cc7133411cde9f27b713a6f59869ec/libyul/optimiser/OrderDependentStateDestroyer.cpp), though not all features are implemented on both branches. Test cases can be found [here](https://github.com/SamWilsn/taint-tests), though not all successfully pass. This prototype is built as a Yul optimization pass, and so requires `--optimize-yul --ir` as additional compiler flags.

Since this is a proof of concept, the output is messy and barely readable, the implementation is inefficient, and is 100% capable of summoning [nasal demons](https://en.wikipedia.org/wiki/Undefined_behavior). Obviously, don’t use this software in any kind of production environment.
...
[![01-split](https://ethresear.ch/uploads/default/original/2X/e/ebc24311042436029561304de3028767a9d7780c.png)01-split475×420 27.8 KB](https://ethresear.ch/uploads/default/ebc24311042436029561304de3028767a9d7780c)
...
Memory accesses are tracked by synthesizing variables (ex. `!!m0`) for every `mstore`, similar to how compilers convert to [single static assignment](https://en.wikipedia.org/wiki/Static_single_assignment_form) form. This process relies on very basic [constant folding](https://en.wikipedia.org/wiki/Constant_folding#Constant_folding). Should an `mstore` or an `mload` access an offset which is not computable at compile time or has not been written to yet, a catch-all variable `!!memory` is used instead.
...
[![02-resolved](https://ethresear.ch/uploads/default/optimized/2X/b/b560a2eec2211e54a0468e7d8bf14a8816c7cb5f_2_456x500.png)02-resolved471×516 36.3 KB](https://ethresear.ch/uploads/default/b560a2eec2211e54a0468e7d8bf14a8816c7cb5f)
...
[![03-taint](https://ethresear.ch/uploads/default/optimized/2X/2/2eac035bea1b6405c479e50d57b4aafe82608aa6_2_457x500.png)03-taint472×516 36.2 KB](https://ethresear.ch/uploads/default/2eac035bea1b6405c479e50d57b4aafe82608aa6)
...
[Constant propagation](https://en.wikipedia.org/wiki/Constant_folding#Constant_folding) is the process of substituting the values of known constants in expressions at compile time.
...
[![04-flow](https://ethresear.ch/uploads/default/original/2X/f/f0271501957fbf366a6bb157952bd4466f70ab8f.png)04-flow182×347 11.5 KB](https://ethresear.ch/uploads/default/f0271501957fbf366a6bb157952bd4466f70ab8f)
...
![05-bytes](https://ethresear.ch/uploads/default/original/2X/6/6e9a3d342176940766864f0688abc61850c64bda.png)
...
[One contract](https://github.com/PhABC/uniswap-solidity/blob/2434367a66b6091db7b808ed91e7ade61fad6f7d/contracts/tokens/ERC20.sol), part of [@PhABC](https://ethresear.ch/u/phabc/summary)’s uniswap-solidity, did successfully compile with minimal modification.
...
Please see [Rice’s Theorem](https://en.wikipedia.org/wiki/Rice's_theorem).
...
> DSA : The set of addresses the transaction might touch to is [0,2160−1][0,2^{160}-1] , or the set of storage items the transaction might touch is [0,2256−1][0,2^{256}-1] . We have no idea what the transaction might try to access.
>
>
> for example: SSTORE[SSLOAD[0]] = 1
...
> However, once your contract calls into another contract we’re back to where we started, it’s going to be very difficult to figure out the set of accounts/storage-slots that the other contract you didn’t write might touch.

This is exactly why, for the [direct push model](https://ethresear.ch/t/state-provider-models-in-ethereum-2-0/6750), we believe we have to eliminate all types of dynamic state access. Obviously arbitrary state access is a problem, but even simple branching is an issue because the set of all possible locations could become quite large compared to the minimal set.

![](https://ethresear.ch/user_avatar/ethresear.ch/lithp/48/1170_2.png) lithp:

> You’re talking about not allowing any flexibility at all! I would be surprised if it was possible to add this restriction without breaking a ton of existing contracts.

That is essentially correct. For new contracts there are [some workarounds](https://ethresear.ch/t/common-classes-of-contracts-and-how-they-would-handle-contract-state-root-plus-witness-architecture/4547), but we aren’t convinced that they are sufficient for a good developer experience. That is what motivated this prototype.

[@adietrichs](/u/adietrichs) and [@villanuevawill](/u/villanuevawill) are manually auditing some contracts to see what changes they’d need to be rewritten without DSA, but it’s unlikely that contracts could be ported without modification.

Getting Eth1 into Eth2 is going to be a whole process on its own, and at least [one of the proposals](https://ethresear.ch/t/alternative-proposal-for-early-eth1-eth2-merge/6666) would require semi-statefulness for Eth1 contracts.
...
> Obviously arbitrary state access is a problem, but even simple branching is an issue because the set of all possible locations could become quite large compared to the minimal set.

Yeah, totally, the set of potential accesses can grow quite quickly.

![](https://ethresear.ch/user_avatar/ethresear.ch/samwilsn/48/4301_2.png) SamWilsn:

> It makes the much smaller claim that compilers can (probably) detect DSA, should we choose to go in that direction.
