...
source: magicians
topic_id: 19547
title: "ERC-7679: userOperationBuilder - a common onchain interface for dapps to interact with 4337 wallets"
author: bumblefudge
date: "2024-04-05"
category: ERCs
tags: [erc]
url: https://ethereum-magicians.org/t/erc-7679-useroperationbuilder-a-common-onchain-interface-for-dapps-to-interact-with-4337-wallets/19547
views: 1788
likes: 6
posts_count: 4
---
...
Opening on behalf of at least two Dereks (Chiang @derekchiang and Rein @arein) who are co-authors on this draft ERC:
      ...
      [github.com/ethereum/ERCs](https://github.com/ethereum/ERCs/pull/361/files)
...
####
      ...
      `master` ← `arein:feat/erc-7777`




          opened 02:05AM - 05 Apr 24 UTC



          [![](https://avatars.githubusercontent.com/u/966091?v=4)
            arein](https://github.com/arein)



          [+223
            -0](https://github.com/ethereum/ERCs/pull/361/files)
...
When opening a pull request to submit a new EIP, please use the suggested templa[…](https://github.com/ethereum/ERCs/pull/361)te: https://github.com/ethereum/EIPs/blob/master/eip-template.md
...
2. Execute a multicall (batched eth_calls) of getNonce, getCallData, getDummySignature with the context and executions.  The building party will now have obtained the nonce, calldata, and dummy signature (see “Rationale” for what a dummy signature is).
...
Note that a UserOp has a lot more fields than nonce, callData, and signature, but how the building party obtains the other fields is outside of the scope of this document, since only these three fields are heavily dependent on the smart account implementation.
...
**danfinlay** (2024-04-10):

Alright, we did some analysis at MetaMask. Looks like we’re willing to support this proposal provided it’s extended to add support for a `redeemDelegation(bytes PermissionsContext, bytes calldata data);` method (described in more detail in [this notion doc](https://www.notion.so/metamask-consensys/Permissions-Standard-2-DApp-Signing-Flow-ae9bc05b38624bc09484a23ec94f70b5)).

We believe this is a necessary addition because the 4337 `isValidSignature` method disallows lookups on external contract state, and so is incompatible with redeeming permissions granted to a contract account (requiring 1271 verification) or whose logic depends on significant amounts of onchain state.

By adding this additional method, contracts would provide two avenues for making their issued `PermissionsContext` blobs consumible: One cheap and gas efficient, and one open ended and powerful.
...
Making sure I understand, you are talking about how, if the permissions are granted to not a key but rather a contract account, then 4337 storage rules may prohibit the `isValidSignature` call on the contract account?

That’s a valid point for sure, but I think this falls under the scope of a permissions standard, not this standard which is specifically for building ERC-4337 UserOps in an account-agnostic way.  Our intention was to propose a separate permissions standard, which would refer to this UserOp-building standard, and which could potentially include the `redeemDelegation` flow as well.

TLDR: this standard is specifically meant for building UserOps in an account-agnostic way, not for permissions.  You will notice that it doesn’t mention the word “permissions” at all.  We should work together on a separate permissions standard, which would refer to this standard.
...
> You will notice that it doesn’t mention the word “permissions” at all.

But it does mention “delegation” of an account’s rights. I think these are basically names for the same thing, and so if we’re going to define a `context` capable of authorizing a secondary signer of building a `UserOp` on behalf of the parent account, then it would be useful to be paired with a method to allow an equivalent `context` to be used by another contract, or redeemed by another account (in the case that the state access is so advanced that it exceeds 4337 bundling requirements).

I can draft this as a separate ERC if you insist these are separate concerns.
