...
tags: []
url: https://ethereum-magicians.org/t/eip-7768-no-ether-transactions-with-free-for-all-tips/21108
views: 138
likes: 1
posts_count: 2
---
...
author: William Entriken ([@fulldecent](/u/fulldecent))
...
None of them;
4. Only the first;
5. The first then the second;
6. The first, then the second, … then the Nth transaction, which is not the last in her series of transactions; or
7. All her transactions, in order.

Alice sends this series of transactions to a service that communicates with block proposers.
...
Step 1: use the actual required gas amount (in Ether).
8. Step 4: abort if the consideration received in this second iteration is not the expected amount from the first iteration.

Use some local business logic to compare the Ether spent in step 1 (second iteration) versus the consideration receieved in step 4 and classify the result as favorable or not.
If the result is favorable, commit this execution context to the mainline. Or if the result is not favorable, rollback this execution context.
...
Continue execution, and publish the block.
