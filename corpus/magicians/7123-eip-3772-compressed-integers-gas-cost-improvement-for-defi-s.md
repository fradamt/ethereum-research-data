...
date: "2021-09-24"
category: EIPs
tags: [smart-contracts, gas-saving]
url: https://ethereum-magicians.org/t/eip-3772-compressed-integers-gas-cost-improvement-for-defi-smart-contracts/7123
views: 915
likes: 4
posts_count: 1
---

# EIP-3772 Compressed Integers (gas cost improvement for DeFi smart contracts)
...
- However, uint256 type allows us to represent $10-18 to $1058, and most of it is a waste. In technical terms, we have the probability distribution for values larger than $1015 and smaller than $10-3 as negligible (i.e. P[val > 1015] ≈ 0 and P[val  In the following code example, uint7 is used just for representation purposes only, but it should be noted that uints in Solidity are in multiples of 8.
...
function compress(uint256 full) public pure returns (uint64 cint) {
    ...
    if (bits > bits) > 8) > 8)  TODO Find more projects and add links here
...
2. Handeling the error
3. Losing precision due to incorrect use of cintx
4. Compressing something other than money uint256s.
