...
source: magicians
topic_id: 10539
title: "EIP-5170: Add Authenticity method for client script"
author: JamesB
date: "2022-08-27"
category: EIPs
tags: [authentication, tokenscript]
url: https://ethereum-magicians.org/t/eip-5170-add-authenticity-method-for-client-script/10539
views: 770
likes: 1
posts_count: 2
---

# EIP-5170: Add Authenticity method for client script
...
Often NFT authors want to provide some user functionality to their tokens through client scripts. This should be done safely, without opening the user to potential scams. Refer to [EIP-5169](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-5169.md) examples of such scripts.
...
- 6: obtain the script itself; in our example it will be bundled in the JWS object itself, along with the signature attached to the script, in the  tag.
- 7: verify that the signature from the  tag is the correct signature of the keccak’d script by the script signing key.
...
1. Example script applied to contract published before this EIP:
...
EIP-5170 makes EIP-5169 more flexible; intended for existing contracts or contracts where the developers were unaware at the time of deploying of EIP-5169 or that they (or other parties) would want the token to have other uses.

For example, we could add an action verb to an existing NFT contract; perhaps the moonbird artists set up a backroom with an electronic lock at a Devcon that could be opened with a moonbird. This could be set up via EIP-5170 as follows:

- QR code on the door which links to an FTP with the door opening script.
- Script is downloaded via an intent which prompts user to pick a supporting wallet.
- The wallet checks that the attached signature is valid for the script and is signed by moonbirds contract owner.
- Display unlock door verb.
