...
source: magicians
topic_id: 20513
title: "ERC-7739: Readable Typed Signatures for Smart Accounts"
author: Vectorized
date: "2024-07-10"
category: ERCs
tags: [account-abstraction, signatures, smart-account]
url: https://ethereum-magicians.org/t/erc-7739-readable-typed-signatures-for-smart-accounts/20513
views: 1730
likes: 10
posts_count: 10
---
...
**Authors:** vectorized ([@vectorized](/u/vectorized)), Sihoon Lee (@push0ebp), Francisco Giordano ([@frangio](/u/frangio)), Im Juno (@junomonster), howydev (@howydev), 0xcuriousapple ([@0xcuriousapple](/u/0xcuriousapple))
...
- Safe: safe-smart-account/contracts/handler/CompatibilityFallbackHandler.sol at 5feb0d08f59cfbb44918be1ed5889d9bb634562a · safe-fndn/safe-smart-account · GitHub
...
Solady Solidity ERC-1271 implementation, which implements this ERC-7739.
      ...
      [github.com/Vectorized/solady](https://github.com/Vectorized/solady/blob/main/src/accounts/ERC1271.sol)
...
####
  ...
  [main](https://github.com/Vectorized/solady/blob/main/src/accounts/ERC1271.sol)
...
```
  ...
  This file has been truncated. [show original](https://github.com/Vectorized/solady/blob/main/src/accounts/ERC1271.sol)
...
https://viem.sh/experimental/solady/hashTypedData
...
Hey [@Vectorized](/u/vectorized)

I have a question regarding `supportsNestedTypedDataSign` applied to modular smart accounts.

I’m working on [Nexus modular smart account](https://github.com/bcnmy/nexus/tree/dev) and also on the [Smart Sessions](https://github.com/erc7579/smartsessions/tree/dev) module.
...
https://github.com/Vectorized/solady/pull/1065
...
I think the fields are necessary, but the extensions are actually a problem. The extensions are not a part of the domain like this ERC is using them, they are an indication of how the domain should be constructed. See [ERC-5267](https://eips.ethereum.org/EIPS/eip-5267#specification):
...
[@Amxx](/u/amxx) points out that this can be addressed by including the EIP712Domain inside `contentsType`. That sounds good.
...
Signing script example:
      ...
      [gist.github.com](https://gist.github.com/Amxx/ffbbdec0e1ea37405045a8241273d3f5)
...
```
   ...
   This file has been truncated. [show original](https://gist.github.com/Amxx/ffbbdec0e1ea37405045a8241273d3f5)
...
**sarareynolds** (2025-04-15):

Is there a reason in the current ERC there are optional fields that are forced to be in the TypedDataSign struct (ie the salt?). It’s just odd because there isn’t really a differentiator then in returning salt thats set to bytes32(0) where its literally used as a salt vs. meaning that its not set at all.

---

**Vectorized** (2025-04-29):

It’s for easier and faster onchain implementation. Cuz otherwise we’ll need to build the string and hash it on-the-fly.
