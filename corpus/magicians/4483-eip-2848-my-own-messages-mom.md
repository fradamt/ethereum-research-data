...
category: EIPs
tags: []
url: https://ethereum-magicians.org/t/eip-2848-my-own-messages-mom/4483
views: 791
likes: 2
posts_count: 5
---
...
MOM want to be a standard to notarize and share messages. This can be achieved just submitting a self-send transaction to the blockchain with a specific format and message data attached.

Your can try a live example here: [MOM client](https://internetofpeers.github.io/mom-client/)
...
I know, My Own Messages (MOM) sounds like *mom* . And yes, pun intended ![:slight_smile:](https://ethereum-magicians.org/images/emoji/twitter/slight_smile.png?v=15)

You can read the full spec below or at [GitHub](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-2848.md)
...
As a *developer* or *pool’s owner*, I’d like to send messages to my users in a decentralized way. They must be able to easily verify my role in the smart contract context (owner, user, and so on) and they must be able to do it without relying on external, insecure and hackable social media sites (Facebook, Twitter, you name it). Also, I’d like to read messages from my userbase, in the same secure and verifiable manner.

As a *user*, I want a method to easily share my thoughts and idea, publish content, send messages, receive feedback, receive tips, and so on, without dealing with any complexity: just write a message, send it and it’s done. Also, I want to write to some smart contract’s owner or to the sender of some transaction.
...
| ENDORSE | 05 | multihash | Endorse a message identified by the specified multihash. The parameter MUST be the multihash of the message to be endorsed. | Clients MUST record and track the endorsement for that specific message. Think it as a like, a retwitt, etc. |
| DISAPPROVE | 06 | multihash | Disapprove a message identified by the specified multihash. The parameter MUST be the multihash of the message to disapprove. | Clients MUST record and track the disapproval for that specific message. Think it as a I don’t like it. |
...
| RAW | FF | any | The parameter MUST be at least 1 byte. Content type is not disclosed and it MUST NOT be considered as text/markdown. | Clients MUST add the message to the message list but they MUST NOT try to decode the content. Clients SHOULD allow users to see this message only if explicitly asked for. This operation can be used for blind notarization that general client can ignore. |
...
Ethereum is *account based*, so it’s good to be identified as a single source of information.
...
There’s no benefit to store *static* messages on-chain, if they are not related to some smart contract’s state or if they don’t represent exchange of value. The cost of storing data on-chain is also very high.
...
The application need to have a secure point-to-point communication is largely filled by TLS - and for good reason. There is also an existing ethereum message passing system called Whisper Wire, which introduced a proof-of-work to prevent message flooding.

The more interesting part here is making it on-chain, and there are difficulties with doing that - so lets discuss it more:



    ![](https://ethereum-magicians.org/user_avatar/ethereum-magicians.org/rook/48/10951_2.png)
    [EIP: Authenticated Whisper Broadcast Messages](https://ethereum-magicians.org/t/eip-authenticated-whisper-broadcast-messages/4476) [EIPs](/c/eips/5)
...
> I am very confused by the name choice for the protocol.  I don’t think “whispering” is an accurate name for a global broadcast.  Additionally, I was first drawn to this project because I thought it was written by Whisper Systems, and now I don’t think they are related at all.
      ...
      [github.com](https://github.com/OpenEthereum/whisper)




  ![image](https://opengraph.githubassets.com/626e7fe876b7083449a5ecdc01556205/OpenEthereum/whisper)
...
Contribute to openethereum/whisper development by creating an account on GitHub.

---

**Neurone** (2020-08-07):

Hi Rook, thanks for feedback but this is not an EIP to create a messaging system like Whisper, it’s meant to notarize messages more like your own certified blog. I’m working to create a better explaination of the EIP and now I divided two sets of operations: core that must be implemented and exteneded, that clients can choose to implement or not

---

**Neurone** (2020-08-07):

You can find a a new version of this EIP on Github and below. I cannot update the first post anymore, I don’t know why, so I post here also:
...
| ENDORSE | 0x06 | multihash | Endorse a message identified by the specified multihash. The parameter MUST be the multihash of the message to be endorsed. | Clients MUST record and track the endorsement for that specific message. Think it as a like, a retwitt, etc. |
...
| DISAPPROVE | 0x08 | multihash | Disapprove a message identified by the specified multihash. The parameter MUST be the multihash of the message to disapprove. | Clients MUST record and track the disapproval for that specific message. Think it as a I don’t like it. |
...
Peep an EIP #8:  EIP-2848 with Giuseppe Bertone
  ...
  [![image](https://ethereum-magicians.org/uploads/default/original/2X/5/55bf2e93c881e1e06cf8b0ee1eb4d5b9a8f21e6c.jpeg)](https://www.youtube.com/watch?v=z1SnoQkQYkU)
