...
category: EIPs
tags: [nft]
url: https://ethereum-magicians.org/t/eip-5896-rejectable-non-fungible-token/11674
views: 709
likes: 0
posts_count: 1
---

# EIP-5896: Rejectable Non-Fungible Token

Pull request: [Add EIP-5896: Rejectable Non-Fungible Token · GitHub](https://github.com/ethereum/EIPs/pull/5896)

## Abstract

This improvement of the EIP-721 includes the possibility that the receiver could reject a token transfer, allowing the selective reception of NFTs. The proposal depends on and extends the existing [EIP-721](https://eips.ethereum.org/EIPS/eip-721).
...
1. We also need to take into consideration the mint() function, where the token is created and doesn’t have yet an owner. In fact, minting a new token represents also a transfer of ownership, from the zero address, to the receiver.
2. With this proposal, the procedure to transfer a token is completed using the newly defined functions that change the values of the _transferableOwners mapping states as follows:
...
1. The transferableOwnerOf() function returns the address of the token to which it is currently offered. If the token is not offered, the zero address is returned.
