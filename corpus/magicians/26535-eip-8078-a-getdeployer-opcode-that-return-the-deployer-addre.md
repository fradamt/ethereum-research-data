...
source: magicians
topic_id: 26535
title: "EIP-8078: A GETDEPLOYER opcode that return the deployer address of a contract"
author: NolanWang
date: "2025-11-12"
category: EIPs
tags: [evm]
url: https://ethereum-magicians.org/t/eip-8078-a-getdeployer-opcode-that-return-the-deployer-address-of-a-contract/26535
views: 149
likes: 1
posts_count: 3
---

# EIP-8078: A GETDEPLOYER opcode that return the deployer address of a contract

## Abstract



      [github.com/ethereum/EIPs](https://github.com/ethereum/EIPs/pull/10780/)
...
####
      ...
      `master` ← `maldiohead:feat/getdeployer-opcode`




          opened 03:32AM - 12 Nov 25 UTC



          [![](https://avatars.githubusercontent.com/u/23315692?v=4)
            maldiohead](https://github.com/maldiohead)



          [+245
            -0](https://github.com/ethereum/EIPs/pull/10780/files)
...
**ATTENTION: ERC-RELATED PULL REQUESTS NOW OCCUR IN [ETHEREUM[…](https://github.com/ethereum/EIPs/pull/10780)/ERCS](https://github.com/ethereum/ercs)**
...
**wjmelements** (2025-11-12):

There are a few reasons nobody has ever wanted this, but I think the main point is that keys are not indefinitely trustworthy:

- they can be stolen
- they can be tricked
- their original owner can betray previously established trust

Besides that, they may have deployed perfectly safe contracts that aren’t applicable to your situation. Therefore I don’t think anyone should ever run arbitrary code just because it was signed by a key previously used by someone you trust.

Even if you did trust a deployer account, you can provide a proof that they deployed the contract by showing how the address was calculated (eg via create2). This is currently the method used on-chain for checking if a caller is a UniswapV3Pool (smart contracts, unlike EOAs, can be trustworthy deployers). Such proofs also do not require accessing state.

So your opcode won’t be enabling any new functionality, and would likely be more expensive computationally than existing alternatives.

---

**NolanWang** (2025-11-13):

I don’t agree with your point. Security is never a single point of failure. Of course, a private key can be stolen; all security measures have their limits. You mentioned that it can provide proof, but it’s not like a whitelist that can be updated at any time. Even if a trusted deployer’s private key is leaked, we can remove them from the whitelist and use a new deployer. I don’t think you grasp the profound significance of this opcode.
