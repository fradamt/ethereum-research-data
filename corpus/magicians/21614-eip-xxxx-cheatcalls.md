...
source: magicians
topic_id: 21614
title: EIP XXXX - Cheatcalls
author: krzkaczor
date: "2024-11-07"
category: EIPs > EIPs interfaces
tags: []
url: https://ethereum-magicians.org/t/eip-xxxx-cheatcalls/21614
views: 193
likes: 2
posts_count: 5
---
...
Cheatcalls EIP is an attempt to standarize JSON-RPC interface for development nodes. The proposal is in pre-draft state (no PR to official repo yet) but it’s already written in a “shape” fitting EIPs and was shared with few people. You can follow previous discussion on the [GitHub](https://github.com/krzkaczor/cheatcalls-eip). We also have a [Telegram chat](https://t.me/+GB1t7LjN56djYmUy) to discuss the propsal. I would love to hear your thoughts!

# Cheatcalls EIP

| eip | title | description | author | type | category | status | created | discussions-to |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| eip-xxxx | Cheatcalls EIP | Standardizing JSON-RPC interface for development nodes | Kris Kaczor <chris@kaczor.io>, Piotr Szlachciak, Emmanuel Antony <emmanuelantony2000@gmail.com> | Standards Track | Interface | Pre-Draft | 2024-11-07 |  |
...
- cheat_setErc20Balance – storage location of a balance for a given account can be discovered by tracing storage slots read during a balance call and then finding the exact slot by checking them one by one. Such approach was used to implement deal in forge-std (#1, #2)
...
**wjmelements** (2024-11-07):

I think `debug_` is a better API category for this than `cheat_`.

---

**krzkaczor** (2024-11-08):

Thanks [@wjmelements](/u/wjmelements) for your comment!

`debug_` prefix is something that I was thinking about as well but the problem is that usage of such methods is not related to debugging anything. One would use them to force a smart contract system into a certain state for whatever reason (manual / automatic testing? demo? debugging?)

---

**wjmelements** (2024-11-08):

![](https://ethereum-magicians.org/user_avatar/ethereum-magicians.org/krzkaczor/48/3883_2.png) krzkaczor:

> the problem is that usage of such methods is not related to debugging anything

I’ve used them for debugging.

- debug_trace(BadBlock,Block,Call,Transaction)
- debug.verbosity
- debug.setHead
- debug.dbGet
- debug.getBadBlocks

While the debug environments don’t currently let you break your own state (the preference is for per-request account overrides), methods that can break things are generally in debug_.

As for what you’re trying to do, I recommend looking into alternatives that are already possible. I already mentioned the overrides, which work for eth_call and debug_traceCall. Additionally I have built a [system for thorough testing](https://github.com/wjmelements/evm?tab=readme-ov-file#configuration-dio) in a mocked environment that already does most of the things you’re trying to do. It runs tests very fast (for me, 120 tests in 56 ms) and it’s easy to generate its configuration files from existing APIs. I would love if you would build on top of that rather than trying to build on top of an execution client. Furthermore I would help you maintain it.

---

**krzkaczor** (2024-11-09):

Ah, I think you refer to [geth’s debug namespace](https://geth.ethereum.org/docs/interacting-with-geth/rpc/ns-debug), this is quite different from what this proposal tries to accomplish. As you mention they don’t allow to control the state of the node but rather inspect it (with some exceptions like `debug.setHead`). `debug_` fits this use case pretty well.

Cheatcalls are not meant to be implement by production nodes such as geth but standarize already existing methods in [Anvil](https://book.getfoundry.sh/reference/anvil/?highlight=evm_#special-methods), [Tenderly](https://docs.tenderly.co/virtual-testnets/admin-rpc) and others. Block scoped overrides are not powerful enough.

> As for what you’re trying to do, I recommend looking into alternatives that are already possible. I already mentioned the overrides, which work for eth_call and debug_traceCall. Additionally I have built a system for thorough testing  in a mocked environment that already does most of the things you’re trying to do. It runs tests very fast (for me, 120 tests in 56 ms) and it’s easy to generate its configuration files from existing APIs. I would love if you would build on top of that rather than trying to build on top of an execution client. Furthermore I would help you maintain it.

I am not sure if I follow. This EIP is about standarizing methods already developed and used in **develpoment nodes**, not testing environments such as `forge test`.

I will tweak the proposal in the near future to make that clear.
