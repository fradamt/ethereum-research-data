...
source: magicians
topic_id: 18999
title: "ERC: Intrinsic RevShare Token"
author: socathie
date: "2024-02-29"
category: ERCs
tags: [erc, token, erc-20]
url: https://ethereum-magicians.org/t/erc-intrinsic-revshare-token/18999
views: 5961
likes: 2
posts_count: 2
---

# ERC: Intrinsic RevShare Token

[github.com/ethereum/ERCs](https://github.com/ethereum/ERCs/pull/284)
...
####
      ...
      `master` ← `IMOLabs:master`




          opened 04:40AM - 29 Feb 24 UTC



          [![](https://avatars.githubusercontent.com/u/22270063?v=4)
            socathie](https://github.com/socathie)



          [+1027
            -0](https://github.com/ethereum/ERCs/pull/284/files)
...
When opening a pull request to submit a new EIP, please use the suggested templa[…](https://github.com/ethereum/ERCs/pull/284)te: https://github.com/ethereum/EIPs/blob/master/eip-template.md
...
**Every compliant contract must implement the `IERC7641`, and ERC-20 interfaces.**
...
/**
 * @dev An interface for ERC7641, an ERC-20 extension that integrates a revenue-sharing mechanism, ensuring tokens intrinsically represent a share of a communal revenue pool
 */
...
The **AltRevToken extension** is OPTIONAL for this standard. This allows accept other ERC-20 revenue tokens (more than ETH) into the revenue sharing pool.

The AltRevToken extension

- Defines a claimableERC20 function to calculate the amount of ERC-20 claimable by a token holder at certain snapshot.
- Defines a redeemableERC20OnBurn function to calculate the amount of ERC-20 redeemable by a token holder upon burn.

```solidity
pragma solidity ^0.8.24;

/**
 * @dev An optional extension of the ERC7641 standard that accepts other ERC-20 revenue tokens into the contract with corresponding claim function
 */
...
**tkstanczak** (2024-08-14):

How do you envision the snapshot mechanism? Do you suggest some preexisting standard for this?

In particular:

Can you provide more details on the expected frequency of snapshots and who has the authority to trigger them?

How does the snapshot mechanism interact with token transfers? For instance, if a user transfers tokens between snapshots, how is their claimable revenue calculated?

Can you provide more details on how the snapshot data is stored and accessed? Is it entirely on-chain, or are there off-chain components?
