...
category: EIPs
tags: []
url: https://ethereum-magicians.org/t/idea-smart-contract-event-hooks-standard/11503
views: 2145
likes: 2
posts_count: 3
---

# IDEA: Smart Contract Event Hooks Standard

I have an idea that I like would like to present for discussion ahead of submitting as an EIP:

Please see the [GitHub repo](https://github.com/orbmis/smart-contract-event-hooks) for more info and a reference implementation.
...
Thanks to innovations such as web3 webhooks, web3 actions, or hal.xyz creating a relayer is easier than ever.
...
pragma solidity >=0.7.0 =0.7.0 =0.7.0 =0.7.0  mapping(uint256 => address)) public publishers;
...
pragma solidity >=0.7.0  address) public hooks;
...
pragma solidity >=0.7.0  currentNonce, "Obsolete hook detected");
...
**Pandapip1** (2022-11-16):

Good idea, but who is paying for the gas?

---

**orbmis** (2022-11-17):

The relayer is paying for the gas.  It is envisioned that the relayer would have a contract that can batch call the subscriber contracts when given a hook and an array of addresses, which would avoid having to send individual transactions to each subscriber contract separately.

The fee that the subscriber contract pays for each Hook event update should be greater than the transaction fee, otherwise relayers are not incentivised, and it should be set high enough to compensate for gas price volatility.  This of course may need to be updated from periodically.

Another and possibly simpler approach is to record the approximate amount of gas used by the subscriber when it receives a hook, and simply refund the gas on top of the fee, to some preset upper-bound to maintain safety.

Another concern is competition between relayer that would effectively drive the gas price to the point where the effective fee is zero.

In considering this, and in order to cultivate and maintain a reliable relayer market, it is recommended that where possible, a subscriber contract implements a logical condition that checks that the gas price of the transaction that is calling the `verifyHook` function to ensure that the gas price does not effectively reduce the fee to zero.

This would require that the smart contract have some knowledge of the approximate gas used by the `verifyHook` function, and checks that the condition `minFee => fee - (gasPrice * gasUsed)`.  This will mitigate against competitive bidding that would drive the effective fee to zero, by ensuring that there is some minimum fee below which the effective fee is not allowed to drop.

This would mean that the highest gas price that can be paid before the transaction reverts is `fee - minFee + ε` where `ε ~= 1 gwei`.  This will require careful estimation of the gas cost of the `verifyHook` function and an awareness that the gas used may change over time as the contract’s state changes.
